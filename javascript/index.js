...
let atIdx = email.indexOf('@'),
    dotIdx = email.indexOf('.');

email = `_${email.slice(0, atIdx)}_@${email.slice(atIdx + 1, dotIdx)}.kr${email.slice(dotIdx)}`;

// let atAr = email.split('@'),
//     dotAr = atAr[1].split('.');

// email = `_${atAr[0]}_${dotAr[0]}.kr.${dotAr[1]}`
