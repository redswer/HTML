...
/* n1은 정의되지 않았으므로 ReferenceError(n1 is not defined)가 발생되어야 함에도 38행의 var로 선언된
      n1에 대한 var hoisting으로 인해 undefined값 반환 */
...
/* var는 함수 스코프(Function Scope)로써 블럭 스코프와 관계없이 함수 외부 또는 내부로 구분짓는
      전역과 지역 변수로 나뉨
   블럭 내부에 선언된 var는 함수 내에서만 지역적인 성격을 띄며 블럭에는
      영향을 받지 않아 블럭을 무시하고 전역 변수로 인식 */

function varTest() {
      var n2 = 10;
}
console.log(n2);
// 함수 안에서 var 가 선언되었기 때문에 컴파일 오류가 뜸

{
      var n3 = 15;
}
console.log(n3);
// 블럭을 무시하고 전역변수로 인식되기 때문에 n3의 값인 15 반환
console.log();


/* var은 이와같이 중복선언도 가능
   var은 유연한 코딩이 가능하지만 코드의 안정성을 저하시키는 요인이 될 수 있으므로
    특수한 경우를 제외하고는 사용을 지양 */
